...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = rest.map(lambda operand: scheme_eval(operand, env, True))
...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    if procedure.use_env:
        args.append(env)
    try:
        final_return = procedure.fn(*[argument for argument in args])
    except TypeError:
        raise SchemeError
    return final_return
    # END Question 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    def get_formals(bindings):
        if bindings is nil:
            return nil
        if bindings.second is nil or isinstance(bindings.second, Pair):
            return Pair(bindings.first.first, get_formals(bindings.second))
    def get_vals(bindings):
        if bindings is nil:
            return nil
        if bindings.second is nil or isinstance(bindings.second, Pair):
            return Pair(scheme_eval(bindings.first.second.first,env), get_vals(bindings.second))

    check_form(bindings.first,2,2)
    formals = get_formals(bindings)
    check_formals(formals)
    vals = get_vals(bindings)
...
def check_formals(formals):
    ...
    if isinstance(formals,Pair):
        while len(formals) != 0:
            if isinstance(formals.first,Pair):
                raise SchemeError
            if scheme_symbolp(formals.first):
                if formals.first in check:
                    raise SchemeError
            else:
                raise SchemeError
            check.append(formals.first)
            formals = formals.second
    else:
